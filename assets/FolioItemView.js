...
ADOBE.FolioItemView = Backbone.View.extend({
	...
	initialize: function() {
		var html  = "<img id='folio-thumb' class='folio-thumb' width='90' height='125' />";
		    html += "<div class='text'>";
		    html +=     "<div class='magazine-title'><%= title %><div class='publication-date'><%= dateLabel %></div></div>";
		    html +=     "<div class='state'></div>";
		    html +=     "<div class='button-row'>";
		    html +=         "<div class='black-button' id='buy-button'></div>";
		    html +=         "<div class='blue-button archive-button'>Archive</div>";
		    html +=     "</div>";
		    html += "</div>";
	...
	render: function() {
		...
		if (ADOBE.isAPIAvailable) {
			...
			this.$el.on("click", ".archive-button", function() { scope.archiveButton_clickHandler() });
	...
	getPreviewImageHandler: function(transaction) {
		if (transaction.state == adobeDPS.transactionManager.transactionStates.FINISHED && transaction.previewImageURL != null) {
			...
			var scope = this;
			setTimeout(function() {
				var transaction = scope.folio.getPreviewImage(90, 125, true);
	...
	archiveButton_clickHandler: function() {
		try {
			if (this.folio.isArchivable) {
				this.$el.find(".archive-button").css("opacity", .6);
